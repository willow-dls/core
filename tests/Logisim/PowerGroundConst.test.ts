...
beforeAll(async () => {
    ...
    const project = await loadProject(LogisimLoader, "tests/Logisim/PowerGroundConst.circ");
...
test("Power not", async () => {
    ...
    const results = circ.run()
    expect(results.outputs.Output3.toString()).toStrictEqual(outputs.Output3.toString());
})
...
test("Ground not", async () => {
    ...
    const results = circ.run()
    expect(results.outputs.Output4.toString()).toStrictEqual(outputs.Output4.toString());
})
...
test("Constant 0", async () => {
    ...
    const results = circ.run()
    expect(results.outputs.Output5.toString()).toStrictEqual(outputs.Output5.toString());
})
...
test("Constant 1 Not", async () => {
    ...
    const results = circ.run()
    expect(results.outputs.Output6.toString()).toStrictEqual(outputs.Output6.toString());
})